#pragma once
/*
 * SPDX-FileCopyrightText: Copyright (c) 2025 Rémi Bourgeois. All rights
...
int matrix_reader(std::string           filename,
                  int&                  n,
                  int&                  nnz,
                  int**                 csr_offsets_h,
                  int**                 csr_columns_h,
                  value_type**          csr_values_h,
                  cudssMatrixViewType_t mview)
{
    ...
    while (std::getline(file, line)) {
        ...
        if (!foundHeader && line.substr(0, 14) == "%%MatrixMarket") {
            ...
            lineData >> marker >> object >> format >> value_type_header >>
                symmetry;
            ...
            if (object != "matrix" || format != "coordinate" ||
                ...
                fprintf(
                    stderr,
                    "ERROR: Expected 'matrix coordinate real for the matrix "
                    "file header\n");
                fprintf(stderr,
                        "Got: object=%s format=%s value_type=%s\n",
                        object.c_str(),
                        format.c_str(),
                        value_type_header.c_str());
    ...
    *csr_values_h  = (value_type*)malloc(nnz * sizeof(value_type));
    ...
    if (file_line_count != declared_nnz) {
        fprintf(stderr,
                "ERROR: more elements in the mtx file than announced in the "
                "header\n");
    ...
    if (!(FoundAUpperElement && FoundAlowerElement) &&
    ...
    while (std::getline(file, line)) {
        ...
        if (!foundHeader && line.substr(0, 14) == "%%MatrixMarket") {
            ...
            if (object != "matrix" || format != "array" ||
                value_type_header != "real") {
                fprintf(stderr,
                        "ERROR: Expected 'matrix array real for the rhs file "
                        "header\n");
                fprintf(stderr,
                        "Got: object=%s format=%s value_type=%s\n",
                        object.c_str(),
        ...
        if (!foundSize) {
            ...
            if (num_rhs != 1) {
                fprintf(
                    stderr, "Only one RHS is supported, %d found\n.", num_rhs);
                return EXIT_FAILURE;
            }
